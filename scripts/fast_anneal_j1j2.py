"""
fast_anneal_j1j2.py — pegasus_fast benchmark for heisenberg_j1j2_1d.

Runs N=8 and N=12 across all J2 values with 3 seeds each, using fixed
sensible hyperparameters (no search).

Budget: ~0.3 s per SR iteration (pegasus_fast cloud overhead)
        2 N × 7 J2 × 3 seeds = 42 runs × 15 iters × 0.3 s ≈ 3 min total
...
# 0.3 s/iter × 15 iters × 42 runs ≈ 189 s ≈ 3 min
N_ITERATIONS      = 150
N_SAMPLES         = 1000
...
if __name__ == "__main__":
    ...
    print(f"pegasus_fast benchmark — {HAMILTONIAN}")
