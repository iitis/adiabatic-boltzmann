"""
Langevin SB sampler sweep.

Sweeps: N=24, h ∈ {0.5, 1.0, 2.0}, lr over LEARNING_RATES, 1 seed.
Sampler: langevin / langevin  (LangevinSampler via the Langevin SB kernel on CPU).

Usage
-----
    cd <repo-root>
    python scripts/exper/experiment_langevin.py             # run everything
    python scripts/exper/experiment_langevin.py --dry-run   # print grid, no execution
"""
...
SAMPLER_TAG = "langevin"
METHOD_TAG = "langevin"

# Langevin SB integration knobs forwarded to the Julia server per-sample call.
...
def make_sampler() -> LangevinSampler:
    return LangevinSampler(
        num_rep=max(1024, FIXED["n_samples"]),
...
def main():
    ...
    print(f"[{datetime.now():%H:%M:%S}] Langevin sweep — {len(grid)} total runs")
    print(f"  N={SIZES}  h={H_VALUES}  lr={LEARNING_RATES}")
    print(f"  Pending: {len(pending)}  skipped: {n_skip}  workers: {cli.workers}\n")

    log_path = Path(__file__).resolve().parent / "experiment_langevin_failures.jsonl"
    n_done = n_fail = 0
