...
CONV_WINDOW = 20
CONV_THRESHOLD = 0.01  # 1% CV of energy-per-spin
...
def parse_file(path: Path) -> dict:
    ...
    sampling_time = np.array(history["sampling_time_s"])
    ...
    return {
        ...
        "avg_sampling_time_s": float(np.mean(sampling_time[sl])),
        "mean_sampling_time_all_s": float(np.mean(sampling_time)),
...
def main():
    ...
    df1d = filter_df(df[(df["model"] == "1d") & df["distance_at_conv"].notna()])
    h_values = [0.5, 1.0, 2.0]

    fig, axes = plt.subplots(2, len(h_values), figsize=(5 * len(h_values), 9), sharey="row")
    ...
    for col, h in enumerate(h_values):
        ...
        ax_top.set_title(f"h = {h}")

        ax_bot = axes[1, col]
        plot_metric(sub[sub["conv_iter"] > 0], "time_to_conv_s", "Time to convergence (s)", ax_bot,
                    log=True, linestyle="--", marker="s")

    for col in range(len(h_values)):
        axes[0, col].set_ylabel(r"Mean $|e_\mathrm{VMC} - e_\mathrm{exact}|$ at convergence")
    ...
    fig.suptitle(
        ...
        f"(conv: CV < {CONV_THRESHOLD*100:.0f}% over {CONV_WINDOW} iters, "
