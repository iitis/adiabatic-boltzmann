#!/usr/bin/env python3
"""
plot_phase_transition_heisenberg.py

Trains a small RBM-VMC on the 1D XXZ Heisenberg chain at three values of the
anisotropy Δ (XY / isotropic / Néel-Ising) and visualises how antiferromagnetic
ordering changes across the BKT transition at Δ_c = 1:

  Row 0: heatmap of spin configurations sampled from the trained RBM
          (rows sorted by staggered magnetisation M_s = Σᵢ (-1)^i σᶻᵢ)
  Row 1: staggered spin-spin correlation C(r) = <σ⁰z σᵣz> vs distance r,
          showing alternating sign and different decay rates per phase,
          VMC samples (solid) vs exact ground state (dashed)

Hamiltonian:  H = J Σᵢ [σˣᵢσˣᵢ₊₁ + σʸᵢσʸᵢ₊₁ + Δ σᶻᵢσᶻᵢ₊₁],  J = 1 (AF)

Phase diagram (J > 0):
    Δ < −1       ferromagnetic (FM), gapped
    −1 < Δ < 1   Luttinger liquid (XY phase), gapless
    Δ = 1        isotropic XXX, BKT critical point
    Δ > 1        Néel / Ising-ordered AF, gapped

Usage (from repo root):
    python scripts/viz/plot_phase_transition_heisenberg.py
    python scripts/viz/plot_phase_transition_heisenberg.py --size 12 --iters 400
"""
...
DELTA_POINTS = [
    (0.5, r"XY / LL  ($\Delta = 0.5$)"),
    (1.0, r"Isotropic XXX  ($\Delta = 1.0$, BKT)"),
    (3.0, r"N\'{e}el / Ising  ($\Delta = 3.0$)"),
]
...
def _build_xxz_sparse(N: int, J: float, delta: float) -> sp.csr_matrix:
    """
    XXZ Hamiltonian H = J Σᵢ [σˣᵢσˣᵢ₊₁ + σʸᵢσʸᵢ₊₁ + Δ σᶻᵢσᶻᵢ₊₁] with PBC.
    Spin convention ±1; bit (N-1-i) of index s encodes spin at site i.
    Off-diagonal: 2J for opposite-spin bonds (from σˣσˣ + σʸσʸ = 2(σ⁺σ⁻ + σ⁻σ⁺)).
    """
    dim = 2 ** N
    ...
    for i in range(N):
        ...
        vals_list.append(np.full(opp.sum(), 2.0 * J))
...
def exact_spin_corr(N: int, J: float, delta: float, max_r: int) -> np.ndarray:
    """C_exact(r) = <ψ₀| σ₀z σᵣz |ψ₀> from Lanczos diagonalization."""
    H = _build_xxz_sparse(N, J, delta)
...
def train_and_sample(
    ...
    model = HeisenbergXXZ1D(size=size, J=1.0, delta=delta)
...
def make_figure(args):
    ...
    cache_path = out / f"heisenberg_xxz_cache_N{N}_nh{args.n_hidden}_it{args.iters}_ns{args.samples}_s{args.seed}.npz"
    ...
    if args.plot_only:
        ...
        phase_data = []
        for delta, _ in DELTA_POINTS:
            print(f"\n── Δ = {delta} ──────────────────────────────")
            V, E_vmc, E_exact = train_and_sample(
                size=N, n_hidden=args.n_hidden, delta=delta,
    ...
    for col, ((delta, label), (V, E_vmc, E_exact)) in enumerate(zip(DELTA_POINTS, phase_data)):
        color = _COLORS[col]

        # Sort by staggered magnetisation M_s = Σᵢ (-1)^i σᶻᵢ
        ...
        c_exact = exact_spin_corr(N, J=1.0, delta=delta, max_r=max_r)
    ...
    for ext in ("pdf", "png"):
        path = out / f"spin_ordering_heisenberg_xxz.{ext}"
...
if __name__ == "__main__":
    ...
    if args.n_hidden is None:
        args.n_hidden = 2 * args.size  # Heisenberg needs more capacity than TFIM
    make_figure(args)
