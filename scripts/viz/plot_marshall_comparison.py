...
def plot(results, out_dir):
    ...
    for (mode, kw, label) in [
        ...
        ax.plot(ratios, means, label=label, **kw)
    ...
    ratios_m, means_m, _ = agg["marshall"]
    above = ratios_m > 0.5
    x_tip = float(ratios_m[above][0]) if above.any() else 0.6
    y_tip = float(means_m[above][0])  if above.any() else 0.05
    ax.annotate(
        "Majumdar--Ghosh\n" + r"($J_2/J_1=0.5$)",
        xy=(x_tip, y_tip * 0.5),
        xytext=(min(x_tip + 0.1, 0.82), y_tip * 0.9),
        arrowprops=dict(arrowstyle="->", color="#444", lw=0.9),
        fontsize=9, color="#333",
    )

    # --- bottom: average sign <s> = 1 - 2*sign_impurity ---
    ax = axes[1]
    ...
    for ext in ("pdf", "png"):
        path = out_dir / f"marshall_comparison.{ext}"
